...
const saltRounds: number = parseInt(<string>process.env.SALT_ROUNDS)
...
export default class {
	...
	static async add(user: User): Promise<boolean> {
		await bcrypt.hash(user.password, saltRounds, async function (err, hash) {
			...
			const stmt = "INSERT INTO User(id,username,name,surname,password) VALUES($id,$username,$name,$surname,$password)"
			try {
	...
	static async exists(column: string, value: string): Promise<boolean> {
		const res: object = <object>await dbService.get(
